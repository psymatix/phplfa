...
class PowerNetwork
{
   ...
   public function busUnknowns($b)
   {
       ...
       // $voltageControlledNums = array(1,2,3,6,7,10,11,14,15);
        ...
        $pUnknown = array(1, 2, 3, 4, 5, 6, 7);
       if (in_array($typeDec, $pUnknown)) {
           $b->unknowns[] = 'P';
       }
       $qUnknown = array(1, 2, 3, 8, 9, 10, 11);
       if (in_array($typeDec, $qUnknown)) {
           $b->unknowns[] = 'Q';
       }
       $vUnknown = array(4, 5, 8, 9, 12, 13);
       if (in_array($typeDec, $vUnknown)) {
           $b->unknowns[] = 'V';
       }
       $dUnknown = array(4, 6, 8, 10, 12, 14);
       if (in_array($typeDec, $dUnknown)) {
           $b->unknowns[] = 'D';
       }

       if (in_array($typeDec, $voltageControlledNums) && ($b->number != $this->slackBus) && ($b->gen)) {
    ...
    public function formAdmittanceMatrix()
    {
      ...
      if (!$this->admittanceMatrix) {
          ...
          if (ksort($this->buses)) {
              foreach ($this->buses as $key1 => $value1) {
                  ...
                  if (ksort($yin) && $value1->type != 'ground') {
                      $admittanceMatrixArray[ intval($value1->number) ] = $yin;
                  }
              }//level 1
   ...
   public function formImpedanceMatrix($source = array())
   {
   ...
   public function busRealPower($Bi)
   {
    ...
    public function busReactivePower($Bi)
    {
    ...
    public function qRegulate($Bi, $Q)
    {
    ...
    public function delPDelQMatrix()
    {
        $this->rmstart('dPdQ');
        $dP = array();
        $dQ = array();
    ...
    public function loadXMLNetwork($src)
    {
       ...
       $xmlbuses = $xml->getElementsByTagName('Bus');
        ...
        foreach ($xmlbuses as $busData) {
           ...
           //use PUQ basemva
          if ($this->basemva) {
              $qMax = ($qMax / $this->basemva);
              $qMin = ($qMin / $this->basemva);
          }
